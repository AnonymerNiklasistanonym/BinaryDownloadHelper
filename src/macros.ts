import type { Variable } from "../schemas/config.schema";
...
export const replaceVariables = (source: string, variables?: Variable[]): VariableReplacedString => {
    let processedString = source;
    const usedVariables: Variable[] = [];
    if (variables) {
        for (const variable of variables) {
            const newString = replaceAll(processedString, `\\\$\\\{${variable.name}\\\}`, variable.value);
